    ...
    try {
      ...
      for (const uid of uids) {
        try {
          ...
          // Приоритет из фильтра валидируем: невалидное значение прервало бы создание задачи (enum Prisma)
          const VALID_PRIORITIES = ['low', 'medium', 'high', 'urgent'];
          let priority = decision.priority || 'medium';
          if (decision.priority && !VALID_PRIORITIES.includes(priority)) {
            console.warn(`[EmailWorker] Invalid priority "${priority}" from filter "${decision.filterName}", fallback to "medium"`);
            priority = 'medium';
          }
          ...
          // Лог фильтра пишется до создания задачи — проставляем в нём id созданной задачи
          if (decision.filterId) {
            await prisma.emailFilterLog.updateMany({
              where: { filterId: decision.filterId, taskId: null },
              data: { taskId: task.id },
            });
          }
          ...
          console.error(`[EmailWorker] Failed to process UID ${uid}:`, err);
        }
      }
    } finally {
      lock.release();
