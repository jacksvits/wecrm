...
export async function authMiddleware(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: decoded.id },
      include: { role: { select: { name: true, allowedPages: true, canAccessChat: true, showFinancesTab: true, canChangeTaskStatus: true, allowedTaskStatuses: true, canCreateNews: true, canEditNews: true, canHandleSpam: true } } },
    });
    if (!user) {
